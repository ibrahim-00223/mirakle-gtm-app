...
export default function DashboardPage() {
  ...
  const totalSent = contacts?.filter((c) => c.outreach_status !== 'pending').length ?? 0
  const totalOpened = contacts?.filter((c) => ['opened', 'replied'].includes(c.outreach_status ?? '')).length ?? 0
  const totalReplied = contacts?.filter((c) => c.outreach_status === 'replied').length ?? 0
